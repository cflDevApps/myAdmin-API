...
import com.cflApps.MyAdmin.services.DepartmentService;
...
@Slf4j
@Service("pdf")
public class PDFProcessor implements ReportProcessor{
	...
	@Autowired
	public PDFProcessor(DepartmentService depService) {
	...
	public byte[] generateFile(RequestReportDTO requestDto) {
		...
		LocalDate dtInit = requestDto.getDtInit();
		LocalDate dtEnd = requestDto.getDtEnd();
		...
		try (InputStream inputStream = template.getInputStream()){
			...
			log.error(msg);
			throw new MyAdminServiceException(msg);
		}		
	}
